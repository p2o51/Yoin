...
为了将 Yoin 中的实体（依赖于 Subsonic 等不同提供商的本地/远端 ID）关联到 NeoDB 中全局统一的实体（以 `uuid` 标识），Yoin 实现了一套完善的搜索和缓存映射体系。

### 6.1 `ExternalMapping` 映射表
- **设计初衷**：在 Yoin 中，相同的音乐可能由于 Provider 的不同（例如：从 Subsonic 切换到了 Spotify 或者其他后端服务）存在多个不同的 `albumId`。Yoin 引入了 `ExternalMapping` 表，作为多对一关联表。
- **存储结构**：
  - 主键（PK）锚定在外部服务侧：`(externalService, externalId, provider, entityType, entityId)`。这意味着同一个 NeoDB 的 uuid 可以拥有多条来自不同 Provider 的 Yoin 本地 ID 的挂载。
  - **唯一索引约束**：`(provider, entityType, entityId, externalService)` 的唯一索引强制了对于某个外部服务（NeoDB），本地的一个特定实体（如 Subsonic 上的专辑 A）**最多只允许映射到一个 `item_uuid`**，杜绝重复推送的幽灵资源。

### 6.2 匹配策略 (`resolveAlbumUuid`)
当试图向 NeoDB Push 或 Pull 某个专辑时，会进入 `NeoDBSyncService.resolveAlbumUuid` 流程：
1. **缓存读取（O(1) 查询）**：首先利用 `provider`、`entityType`（"album"）和 `entityId` 访问 `ExternalMappingDao` 查询是否已有映射缓存。有则直接复用，从而避免重复消耗网络请求。
2. **搜索兜底与拼装规则**：如果没有缓存，通过专辑名称和作者组合查询（即：`"AlbumName ArtistName"` 拼接并 `trim` 后）调起 API 的 `searchAlbum`。
3. **缓存回写**：API 返回搜索结果后，截取匹配列表里的第一个结果（`hit.uuid`），并将它持久化回写至 `ExternalMapping` 表中，以加速下一次同步效率。

### 6.3 搜索回退策略（Fallback）
在 API 层面上 (`NeoDBApi.searchAlbum`)，NeoDB 的类别标签在版本迭代中发生了变更，为此加入了鲁棒的容错回退机制：
- **默认策略**：优先携带参数 `category=music` 检索 `api/catalog/search`。
- **错误拦截与降级重试**：
  - 如果抛出 `NeoDBException`，并且检测到 HTTP 状态码为 `422` 且错误体包含了 `category` 相关的字段时，触发降级重试。
  - 拦截该错误并输出警告日志（"...retrying legacy category=album"），接着以旧版本的 `category=album` 再次尝试搜索。
- 这一机制确保了即使连接到的自建 NeoDB 实例没有升级到最新接口规范，客户端依然能够保证实体匹配成功。
